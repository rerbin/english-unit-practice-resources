...
public class WrongBookDb {
    ...
    public synchronized String list(String stage,String filter,int limit,int offset) throws Exception {
        ...
        JSONArray out=new JSONArray();String sql="SELECT m.*,ci.audio_key AS audio_key FROM mistakes m LEFT JOIN content_items ci ON ci.id=m.source_item_id WHERE "+where.toString().replace("stage=","m.stage=").replace("pronunciation_error","m.pronunciation_error").replace("writing_error","m.writing_error")+" ORDER BY m.updated_at DESC,m.id DESC LIMIT ? OFFSET ?";args.add(String.valueOf(limit));args.add(String.valueOf(offset));SQLiteDatabase db=getReadableDatabase();
        try(Cursor c=db.rawQuery(sql,args.toArray(new String[0]))){while(c.moveToNext()){JSONObject o=row(c);JSONArray words=new JSONArray();try(Cursor w=db.rawQuery("SELECT word_index,word_text FROM mistake_words WHERE mistake_id=? ORDER BY word_index",new String[]{String.valueOf(c.getLong(c.getColumnIndexOrThrow("id")))})){while(w.moveToNext()){JSONObject z=new JSONObject();z.put("index",w.getInt(0));z.put("word",w.getString(1));words.put(z);}}o.put("words",words);int akc=c.getColumnIndexOrThrow("audio_key");o.put("audioKey",c.isNull(akc)?"":c.getString(akc));out.put(o);}}
