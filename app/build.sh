...
/run/csi/mount-root/nas/4079184d856ecc166ed19d4887083405/tools/jdk/bin/javac -source 8 -target 8 -encoding UTF-8 -classpath "$ANDROID:$P/libs/vosk-classes.jar:$P/libs/jna-classes.jar:$P/libs/sherpa-classes.jar" -d "$P/build/classes" $(find "$P/src" -name '*.java')
$D8 --lib "$ANDROID" --min-api 30 --output "$P/build/dex" $(find "$P/build/classes" -name '*.class') "$P/libs/vosk-classes.jar" "$P/libs/jna-classes.jar" "$P/libs/sherpa-classes.jar"
cp "$P/build/base.apk" "$P/build/unsigned.apk"
python3 -c "import zipfile,os;z=zipfile.ZipFile('$P/build/unsigned.apk','a');z.write('$P/build/dex/classes.dex','classes.dex',zipfile.ZIP_DEFLATED);lib='$P/libs/jni/arm64-v8a/libsherpa-onnx-jni.so';
import sys
if os.path.exists(lib): z.write(lib,'lib/arm64-v8a/libsherpa-onnx-jni.so',zipfile.ZIP_STORED)
z.close()"
