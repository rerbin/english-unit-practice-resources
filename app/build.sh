...
/run/csi/mount-root/nas/4079184d856ecc166ed19d4887083405/tools/jdk/bin/javac -source 8 -target 8 -encoding UTF-8 -classpath "$ANDROID:$P/libs/vosk-classes.jar:$P/libs/jna-classes.jar" -d "$P/build/classes" $(find "$P/src" -name '*.java')
$D8 --lib "$ANDROID" --min-api 30 --output "$P/build/dex" $(find "$P/build/classes" -name '*.class') "$P/libs/vosk-classes.jar" "$P/libs/jna-classes.jar"
